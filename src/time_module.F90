...
CONTAINS
  ...
  SUBROUTINE set_calendar(CalendarString)
    ...
    ELSEIF (TRIM(CalendarString) == "noleaps") THEN
      Calendar = NoLeaps
    ELSE
      WRITE(ERROR_UNIT,*) CalendarString//" is not a recognised calendar."
      STOP 5
    END IF
  ...
  FUNCTION time_as_string(Year, Month, Day, Hour, Minute, Second)
    !*## Purpose
    !
    ! Convert a specified time to a YYYY-MM-DD hh:mm:ss string.
    INTEGER, INTENT(IN) :: Year, Month, Day
    INTEGER, INTENT(IN), OPTIONAL :: Hour, Minute, Second
    CHARACTER(LEN=19) :: time_as_string
    INTEGER :: Hour_, Minute_, Second_
    ...
    WRITE(time_as_string(1:4),'(I0.4)') Year
    WRITE(time_as_string(5:5),'(A)') "-"
    WRITE(time_as_string(6:7),'(I0.2)') Month
    WRITE(time_as_string(8:8),'(A)') "-"
    WRITE(time_as_string(9:10),'(I0.2)') Day
    WRITE(time_as_string(11:11),'(A)') " "
    WRITE(time_as_string(12:13),'(I0.2)') Hour_
    WRITE(time_as_string(14:14),'(A)') ":"
    WRITE(time_as_string(15:16),'(I0.2)') Minute_
    WRITE(time_as_string(17:17),'(A)') ":"
    WRITE(time_as_string(18:19),'(I0.2)') Second_

  END FUNCTION time_as_string
  ...
  ! These procedures are currently unnecessary, but I'm hesitant to delete them
  ! yet as they may be needed later.
